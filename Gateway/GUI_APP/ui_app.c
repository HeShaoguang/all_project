...
static void back_btn_event_handler(lv_obj_t * obj,lv_event_t event)
{
	...
	{
		lv_label_set_text(obj,"#444444 <back#");//使能文本颜色变灰,有按下的效果
...
lv_obj_t * general_win_create(lv_obj_t * parent,const char * title)
{
	...
	lv_page_set_sb_mode(win,LV_SB_MODE_OFF);//设置滚动条的模式为自动
	...
	lv_obj_align(title_label,NULL,LV_ALIGN_IN_TOP_RIGHT,0,TOP_OFFSET);//设置对齐方式
	//创建返回按钮
	lv_obj_t * back_btn = lv_label_create(win,NULL);//用标签来模拟按钮
	lv_label_set_text(back_btn,"< " MY_FAN MY_HUI);//设置文本
