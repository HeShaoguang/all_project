...
afAddrType_t GuangApp_DstAddr;   //点播
afAddrType_t GuangApp_Broadcast;   //广播


#define GUANG_APP_TX_MAX  20    //串口数据最大  大小
static uint8 GuangApp_TxBuf[GUANG_APP_TX_MAX+1];   //串口数据
static uint8 GuangApp_TxLen;   //串口数据大小
...
void GuangApp_MessageMSGCB( afIncomingMSGPacket_t *pkt )
{
  ...
  {
    case GENERICAPP_CLUSTERID:   //点播收到信息
...
void GuangApp_SendTheMessage( void )
{
  ...
  DHT11(); //获取温湿度 
    ...
    {
      if(Feng_Work_State == 0) FENG = 0;          //开蜂鸣器
      //if(Fans_Work_State == 0) FANS = 1;          //开风扇
      light_mq[0] = 0x31;
      LCD_P16x16Ch(80,2,8*16);
    }else
    {
      if(Feng_Work_State == 0) FENG = 1;         //关蜂鸣器
       ...
       if(port==1)
        {
          if (GuangApp_NwkState == DEV_ZB_COORD)
          {
            ...
            if( AF_DataRequest( &GuangApp_Broadcast,         //发送目的地址
                       &GuangApp_epDesc,                    //源
                       GENERICAPP_BROADCASTID,       //集群号
                       GuangApp_TxLen,                             // 发送数据长度
                       GuangApp_TxBuf,                  // 发送数据缓冲区
                       &GuangApp_TransID,     // 任务ID号
